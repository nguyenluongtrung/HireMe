...
import { GetKnowledgeItemDetailResDTO, GetKnowledgeItemsQueryDTO, GetKnowledgeItemsResDTO, UpsertKnowledgeItemBodyDTO, UpsertKnowledgeItemResDTO } from './knowledge-hub.dto';
...
export class KnowledgeHubController {
    ...
    findById(@Param('itemId') itemId: string){
        return this.knowledgeHubService.findById(
            Number(itemId)
        )
    }
    ...
    delete(@Param('itemId') itemId: string){
        return this.knowledgeHubService.delete(
            Number(itemId)
        )
    }
}
